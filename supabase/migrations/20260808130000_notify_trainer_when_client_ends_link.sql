-- Keep the client-initiated link termination atomic: the relationship, audit
-- event, operational-notice revocation, discovery opt-out, and trainer Inbox
-- notification either succeed together or none are persisted.

-- Older local development snapshots predate localized Inbox templates. The
-- shared environment already uses these columns; the guards keep the migration
