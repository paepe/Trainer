...
- [ ] Reconciliar o histórico de migrations do ambiente pré-release antes de qualquer nova promoção de schema. **Observação material 2026-08-10:** a consulta agregada confirmou que `trainer_invitation_operation_events` está disponível e registrou eventos no intervalo, mas `supabase migration list --linked` não contém as migrations locais do ciclo de vida/telemetria (`20260807000000`–`20260808150100`). Isto evidencia deriva no ledger de migrations, não uma falha de disponibilidade do contrato observado. Não foram alterados schema, dados, limites comerciais, entitlement ou comportamento em produção; a reconciliação deve preservar o histórico aplicado e ser validada em pré-release antes de nova promoção.
