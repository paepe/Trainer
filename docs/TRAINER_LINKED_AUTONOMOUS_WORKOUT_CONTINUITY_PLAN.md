...
- [x] Aplicar `workout.sessions_per_week` e `workout.exercises_per_session` na mesma autoridade para ambos os pontos de entrada. Evidência: `generate-smart-workout` e `generate-workout` usam `resolveUserEntitlements`; `resolveAuthoritativeTaskGates` limita exercícios e `countSessionsThisWeek` aplica a quota semanal.
