...
- [x] Resolver o destino de `workout.exercises_per_session` e `workout.exercise_type` conforme decidido na Fase 0 — **já resolvido lá**, não em aberto: `free.workout.exercises_per_session = 6` e `free.workout.exercise_type = 0 (fitness only)` foram confirmados em produção em 2026-08-02. **Estado posterior:** a Fase 5 do plano de licenciamento retirou `workout.exercise_type` como gate comercial em 2026-08-05 (`fitnessOnly = false`); o valor permanece apenas legado. Consultar `FEATURE_ACCESS_MATRIX.md` §4 para o estado actual.
