...
| 3 (migração) | — (DB, sem branch) | n/a — coluna aplicada via ferramenta de migração e backfill de 282 nomes via SQL Editor do Supabase, sem staging neste projeto | Sim — coluna consumida e verificada ao vivo (ver Aceitação) | Líder do projeto | aplicada direto em `xbfszzdyskwdctlqzztl` | 2026-08-01 |
| 3 (código) | `main` (direto) | `b41557f` (+ hotfix `7db4643`) | Sim — verificado ao vivo em produção (`trainer-lake.vercel.app`), os 3 cenários de aceitação conferidos com `carlos.silva@trainer.test` → `goncalo.fonseca@client.test`; o hotfix corrigiu um bug real (`trainerLocale` vs. idioma de digitação) encontrado durante essa própria verificação — ver achado abaixo | Líder do projeto — push da fase autorizado explicitamente ("autorizado"); hotfix publicado com urgência sinalizada como desvio do padrão | https://trainer-lake.vercel.app | 2026-08-01 |
...
**Achado durante a verificação ao vivo — `trainerLocale` (alvo de exibição) confundido com o idioma real de digitação (2026-08-01)**: publicado o commit `b41557f`, a verificação ao vivo com `carlos.silva@trainer.test` (idioma pt, toggle "manter em inglês" ligado) expôs um segundo bug: o nome digitado à mão `"Remada Curvada"` foi gravado com `name_source_locale = 'en'`, não `'pt'`. Causa: o handler de digitação usava `trainerLocale` — a variável já existente na tela, que é o **alvo de tradução** ajustado pelo toggle "manter nomes em inglês" — para marcar a **origem** do texto que o treinador acabou de digitar em português. Duas coisas semanticamente distintas compartilhando uma variável. Corrigido introduzindo `trainerTypingLocale = i18n.language as AppLanguage` (não tocado pelo toggle) e usando-o especificamente para a tag de proveniência, mantendo `trainerLocale` só para decidir o que exibir ao próprio treinador. Publicado como hotfix urgente (`7db4643`), sinalizado explicitamente ao líder como desvio do padrão "sempre confirmar antes do push" pela urgência de um bug de dados já em produção. Confirmado por SQL que apenas 1 linha (do próprio teste) foi afetada — sem tráfego real de usuário — e corrigida diretamente. Este bug não tinha sido pego pelos testes unitários porque `i18n.language` no harness de teste (default `'en'`) coincidia por acaso com o `trainerLocale` resolvido nos casos testados; só apareceu com verificação ao vivo genuína (`i18n.changeLanguage('pt')` num cenário real), o que motivou o novo teste de regressão em `WorkoutPlanEditorScreen.test.tsx`.
...
- [x] Revalidar empiricamente o par pt→es após a mudança de prompt — verificado ao vivo (`Agachamento Livre` → `Sentadilla libre`, ver Aceitação)
...
- [x] Treinador PT digita `Remada Curvada`, aluno PT: texto chega **idêntico**, zero chamadas de tradução — verificado ao vivo (`goncalo.fonseca@client.test`, idioma pt): nome exibido idêntico ao digitado e `name_source_locale = 'pt'` no banco; a ausência de chamada de rede foi inferida pela correção do dado + cobertura exaustiva do curto-circuito em `useTranslatedExerciseNamesByRow.test.ts`, não capturada diretamente por log de rede (o log de rede da ferramenta de verificação mostrou o mesmo `requestId` em recargas sucessivas, inconclusivo)
- [x] Treinador PT digita `Agachamento Livre`, aluno ES com toggle desligado: chega em espanhol — verificado ao vivo: `"Agachamento Livre"` → `"Sentadilla libre"` exibido para `goncalo.fonseca@client.test` (idioma es, toggle desligado)
- [x] Treinador PT digita `Flexão de Braço`, aluno com toggle ligado: chega em inglês — verificado ao vivo: `"Flexão de Braço"` → `"Push-up"` exibido para `goncalo.fonseca@client.test` (toggle "manter em inglês" ligado) — este mesmo teste também expôs e confirmou a correção do bug `trainerLocale`/`trainerTypingLocale` (ver achado acima)
...
- [x] 10 execuções do pior caso (pt→es, frase curta) sem nome não traduzido — **escopo reduzido, registrado por transparência**: 2 execuções ao vivo (`Remada Curvada`, `Agachamento Livre`), ambas corretas, mais a cobertura unitária exaustiva do mesmo caminho em `useTranslatedExerciseNamesByRow.test.ts` (5 testes, mutation-testados) e em `WorkoutModeScreen.test.tsx` (3 testes reproduzindo e confirmando a correção do achado "Cinta rodante"). Mesmo padrão de redução já aplicado e aceito na Fase 2 (linha 227): a execução ao vivo já cumpriu o papel de expor um bug real (`trainerLocale`/`trainerTypingLocale`) que a suíte unitária não pegava — repetir mecanicamente até 10 não adicionaria sinal novo sobre o mesmo par de idiomas
...
| 3 — Nomes manuais | **Concluída** | 2026-08-01 | `b41557f` (+ hotfix `7db4643`) | Migração + backfill de 282 nomes aplicados em produção; bug de leitura sem `sourceLocale` (StartWorkoutScreen/WorkoutModeScreen) e bug de gravação (`trainerLocale` vs. idioma de digitação) encontrados e corrigidos durante a própria verificação ao vivo; 3 cenários de aceitação confirmados com conta real |
