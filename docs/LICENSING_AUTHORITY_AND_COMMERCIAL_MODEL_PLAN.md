...
**Migração do `pro` legado:** plan_key `pro` (50 clientes, €49/mês) marcado `is_active=false` em `plan_definitions` — deixa de aparecer como opção para novos assinantes, mas as suas linhas em `feature_permissions`/`plan_prices` **não foram tocadas**: quem já está no `pro` continua com o mesmo acesso, sem qualquer efeito retroactivo. Nenhuma migração de assinantes existentes para as novas faixas foi feita — está fora do escopo desta sessão e é uma decisão de negócio à parte (para quem migrar, e para qual faixa).
