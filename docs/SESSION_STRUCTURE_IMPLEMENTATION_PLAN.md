...
**Status: implemented and fully verified.** Translation confirmed live for both `exercise_name` and `notes`, at both render sites, with a client profile genuinely set to a different language. Cache read, write, and hit-skip confirmed live in production. Promoted: `24ffbea`.
