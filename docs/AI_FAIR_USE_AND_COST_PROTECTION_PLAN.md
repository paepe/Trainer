...
- [ ] Validar método, `Content-Type`, esquema, tamanho máximo do body/campos, timeout e concorrência por request em todos os endpoints. **Atualização 2026-08-06:** os oito endpoints já rejeitam body acima de um teto global antes de I/O subsequente; permanecem a uniformização de esquema e o controle pré-auth de rajada.
...
| 1 — Exposição imediata | 🟨 Em execução | 2026-08-06 | Os oito endpoints de IA exigem identidade, `Content-Type: application/json` e rejeitam body acima de um teto global antes de I/O subsequente; voz exige entitlement próprio, classificação exige TRAINER, tradução limita fan-out a 8 e valida lote, e as duas rotas de geração aceitam no máximo 128 mil caracteres. Welcome usa idempotência atómica HMAC ativa em produção. CORS de chamadas autenticadas foi validado em produção. Restam uniformização de esquema e a camada pré-auth de rajada. |
