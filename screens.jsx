...
function LoginScreen({ nav, t, setUser, dark }) {
  const [u, setU] = React.useState('frances');
  const [p, setP] = React.useState('••••••••');
  const [err, setErr] = React.useState('');
  const submit = () => {
    if (!u || !p) { setErr('Please fill in both fields.'); return; }
    setUser({ name: 'Frances Scott', email: 'frances@trainer.app', role: 'Client' });
    nav('profile');
  };
  const oauth = (provider) => {
    const profile = provider === 'google'
      ? { name: 'Frances Scott', email: 'frances.scott@gmail.com', role: 'Client' }
      : { name: 'Frances Scott', email: 'frances@icloud.com',     role: 'Client' };
    setUser(profile);
    nav('profile');
  };
  return (
    <div style={{ padding: '24px 28px 28px', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <PillInput icon="user" placeholder="Username" value={u} onChange={setU} primary={t.primary} dark={dark}/>
      ...
      <button onClick={submit} style={primaryBtn(t.primary, dark)}>Log In</button>
...
// ─────────── 3. REGISTER ───────────
function RegisterScreen({ nav, t, setUser, dark }) {
  ...
  const submit = () => {
    ...
    setUser({ name: name || 'New user', email: `${name.toLowerCase()}@trainer.app`, role: 'Client' });
    nav('onboarding');
  };
  const oauth = (provider) => {
    const profile = provider === 'google'
  ...
  return (
    <div style={{ padding: '24px 28px 28px', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <PillInput icon="user" placeholder="Username" value={name} onChange={setName} primary={t.primary} dark={dark}/>
        <PillInput icon="lock" placeholder="Password" value={pw} type="password" onChange={setPw} primary={t.primary} dark={dark}/>
        <PillInput icon="lock" placeholder="Confirm Password" value={pw2} type="password" onChange={setPw2} primary={t.primary} dark={dark}/>
      </div>
      {err && <div style={{ color: t.accent, fontSize: 12, marginTop: 10 }}>{err}</div>}
      <div style={{ flex: 1, minHeight: 12 }}/>
      <button onClick={submit} style={primaryBtn(t.primary, dark)}>Register</button>
