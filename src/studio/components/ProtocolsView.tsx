...
interface ProtocolsViewProps {
  data: {
    ...
    addProtocolExercise: (
      protocolId: string,
      exercise: Omit<ProtocolExercise, 'id' | 'protocol_id'> & { order_index: number }
  ...
  const visible = React.useMemo(() => {
    return data.protocols
      ...
      .sort((a, b) => {
        ...
        if (sortBy === 'level') {
          va = LEVEL_ORDER[a.level || ''] ?? 0;
          vb = LEVEL_ORDER[b.level || ''] ?? 0;
        } else if (sortBy === 'duration') {
          va = a.duration_minutes ?? 0;
          vb = b.duration_minutes ?? 0;
        } else if (sortBy === 'exercises') {
          va = a.exercises?.length ?? 0;
          vb = b.exercises?.length ?? 0;
        } else {
        ...
        return sortDir === 'asc' ? (va as number) - (vb as number) : (vb as number) - (va as number); // Wait, sort order logic
      });
