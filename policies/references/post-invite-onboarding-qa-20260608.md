...
- [ ] Vincular manualmente a conta de teste `paulo.peres@sevenseeds.biz` ao treinador (para destravar o reteste sem esperar um novo convite) — **ou** reenviar um novo convite e validar o fluxo corrigido ponta a ponta (preferível, pois testa a correção real)
- [ ] Reteste: confirmar que a conta aparece como aluno vinculado e que o roteamento server-side funciona mesmo simulando o desvio de recuperação de senha em nova aba
- [ ] **Critério de saída:** vínculo confirmado no banco e refletido na sessão do app, com a correção validada empiricamente (não só por inspeção de código)
